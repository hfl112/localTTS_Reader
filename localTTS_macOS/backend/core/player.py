...
class PCMPlayer:
    ...
    def __init__(self, sample_rate: int = 24000) -> None:
        ...
        self.currently_playing_index: Optional[int] = -1
                ...
                while filled < frames:
                    try:
                        ...
                        if isinstance(item, tuple) and len(item) == 2:
                            samples, chunk_idx = item
                            self.currently_playing_index = chunk_idx
                        else:
                            samples = item
    ...
    def play_chunk(self, chunk: np.ndarray | tuple[np.ndarray, int]) -> None:
